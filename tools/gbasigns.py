...
SIGNS = [
    # (cells, expected vanilla-era blocks, text, style)
    ([(8, 11), (9, 11)], None, "BLANCHE", "board"),
    ([(16, 16)], None, "LAB", "board"),
    ([(5, 14)], None, "?", "post"),
]
...
def main():
    ...
    st = Image.open(os.path.join(SD, "tiles.png"))
    ...
    town = cell(9, 11)
    if WRITE and cell(8, 11) == town:
        raise SystemExit("  already drawn: the town sign already spans (8,11)")
    ...
    new_blocks = 0
    for cells, _, text, style in SIGNS:
        ...
        for i, (x, y) in enumerate(cells):
            ...
            for q in range(4):
                px = [art[(q // 2) * 8 + k // 8][i * 16 + (q % 2) * 8 + k % 8] for k in range(64)]
                e[4 + q] = ((put(px) + 640) | (ROW << 12)) if any(px) else 0
