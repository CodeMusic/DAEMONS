...
#  T-200: the same eighteen colours are now read by the Index entry as well as the move menu, so they live in
#  their own header. One table, two screens -- a type cannot be one colour in battle and another in the Index.
COLOURS = os.path.join(GBA, "src/data/type_colours.h")
RINGS = os.path.join(GBA, "src/data/type_ring_colours.h")   # T-201: the Index category page's pulse
...
def main():
    ...
    #  Two screens read this now: the move menu (a move's name) and the Index entry (the type as a word).
    #  It is static in a header on purpose -- each translation unit gets its own 36 bytes, which is cheaper
    #  than an extern and keeps the generator the only writer.
    K = ["// GENERATED by tools/gbamovemenu.py -- do not edit by hand.",
         "//",
         "// EIGHTEEN TYPES AS EIGHTEEN WORDS. vision.md 9.4 says colour carries the argument; this is the",
         "// table that lets a screen say so in TEXT rather than in a sprite. Each is %s"
         % ("the lightest step of the type's ramp that still clears 4.5:1 on a light box."
            if LIGHTEST else "step 4 of the type's ramp."),
         "//",
    ...
    for key, ours, (r, g, b), how in colours:
        K.append("    [TYPE_%-8s] = RGB(%2d, %2d, %2d), // %s%s" % (key, r, g, b, ours,
                 "" if how in ("step 4", "hue") else " -- " + how))
    K += ["};", "", "#endif // GUARD_DATA_TYPE_COLOURS_H", ""]
    ctext = "\n".join(K)
    cold = open(COLOURS, encoding="utf-8").read() if os.path.exists(COLOURS) else None
    print("  colours: %s" % ("unchanged" if cold == ctext else "would change" if not WRITE else "written"))
    if WRITE and cold != ctext:
        open(COLOURS, "w", encoding="utf-8").write(ctext)

    # ------------------------------------------------ the category page's ring (T-201)
    #  The text table is a DARK step, chosen to be read on a light box. A ring that pulses wants the hue at full
    #  strength, and a highlight lifted toward white for its inner line -- so it is a second table, in its own
    #  header, read only by the Index. Same source: a ring and a routine's name cannot disagree about a type.
    R = ["// GENERATED by tools/gbamovemenu.py -- do not edit by hand.",
         "//",
    ...
    for key, ours in tn.items():
        hue = shipped.get(ours, C.PLAIN_TEXT)
        lit = tuple(min(255, int(v + (255 - v) * 0.55)) for v in hue)
        (r, g, b), (r2, g2, b2) = rgb5(hue), rgb5(lit)
        R.append("    [TYPE_%-8s] = { RGB(%2d, %2d, %2d), RGB(%2d, %2d, %2d) }, // %s%s"
                 % (key, r, g, b, r2, g2, b2, ours, "" if ours in shipped else " -- no hue: the box's grey"))
    R += ["};", "", "#endif // GUARD_DATA_TYPE_RING_COLOURS_H", ""]
    rtext = "\n".join(R)
    rold = open(RINGS, encoding="utf-8").read() if os.path.exists(RINGS) else None
    print("  rings: %s" % ("unchanged" if rold == rtext else "would change" if not WRITE else "written"))
    if WRITE and rold != rtext:
        open(RINGS, "w", encoding="utf-8").write(rtext)
