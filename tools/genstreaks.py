...
writes the two tables it reads.
  ...
  gStreakColours[BODY][MOVE]           the streak a MOVE type draws on a BODY type. Its LIGHT step -- unless
                                       that sits within 9.4's separability floor (CIE76 20, measured on the
                                       body's mid tone) of the body, and then whichever of its light and DARK
                                       steps stands further off it. 9.4 first said a streak in the body's own
                                       type "reads as a vein" at the light step; the preview disproved it for
                                       the low-colour types, where CONTENT's light step IS its body. The
...
def main():
    ...
    for body, _ in order:
        mid = ramps[body][2] if ramps[body] else BLANK
        for move, _ in order:
            if not ramps[move]:
            ...
            pick = light
            if cie76(light, mid) < FLOOR:
                pick = max((light, dark), key=lambda c: cie76(c, mid))
                darkened.append((body, move, cie76(light, mid), cie76(pick, mid)))
            if cie76(pick, mid) < FLOOR:
                tight.append((body, move, cie76(pick, mid)))
    ...
    print("  %d of %d body/move pairs take the DARK step, because the light one is under CIE76 %.0f from the body:"
          % (len(darkened), len(order) * len(order), FLOOR))
    for body, move, before, after in darkened:
        print("     %-9s streak on a %-9s body: light %4.1f -> %4.1f" % (move, body, before, after))
    if tight:
    ...
    text = ("// GENERATED by tools/genstreaks.py -- edit gbasprite.py's TYPE_COLOR or the art, then re-run. (9.4, T-132)\n"
            ...
            "// into a body of that type (CIE76 under %.0f on the body's mid tone).\n\n" % FLOOR +
