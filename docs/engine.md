...
***Symptom (2026-09-25, T-284):*** *Scorn was placed in the Rocket Warehouse on 2026-09-10 "five tiles from Ty", on (22,6) — the single tile of the doorway into Ty's room. An object with no flag never moves, so for fifteen days nobody could reach Ty, the PAYLOAD or the ending after it.* **Nothing complained: the DEBUG kit and JUMP go round the story, and every build, check and fresh clone passed.** *Vanilla does it too, harmlessly: the CONDOMINIUMS 3F's Designer seals off the back wall, and `gbadocs.py` had reserved a document's spot there.* ***Run `python3 tools/check_reach.py` after moving or adding anyone*** — *it walks every map from where a player arrives and reports what is unreachable here and was not in pristine upstream. It knows nothing of Surf, Cut or ledges, so it is only ever read as a difference.* **And the save trick that puts a player on a map without walking there (write the location, save, continue) restores the OLD map's view and people around them** — *good enough for the 3F, useless in the warehouse, where the player could not move at all.* ***Use the theatre warp instead*** *(T-287, section 5).*

### 34. A generator whose --write broke while its report kept passing

***Symptom (2026-09-25, T-291):*** *`gbaowslots.py --write` had stopped at a NameError for four days — `gbasprite.py`'s `ramp5` gained `INK` (T-184) and this tool copies `ramp5` out by AST without it.* **`check_generators.py` runs every tool in REPORT mode, and this tool's report never builds a palette**, *so it passed every time. The overworld daemons kept the outline T-184 had replaced everywhere else.* ***Run `check_generators.py --writes` before relying on a tool's write*** (T-292) — *and when it has been a while, expect it to be stale in both directions: it may fail, or it may succeed and undo what came after it (trap 32; this one would have deleted two graphics ids that others added below its block).* **The 14 that would undo later work now refuse a whole-tree `--write` themselves** (`tools/driftguard.py`, reading `generator_drift.json`; T-293). ***And drift is read both ways***: *of the 24 T-292 found, four were only line endings (a JASC palette written LF where `.gitattributes` checks it out CRLF), four were a later pass the tool could learn (`gbaoutline`'s outline; a released animation it took for vanilla), and two were the TREE behind the tool — `streaks.h` never regenerated after twelve daemons were drawn, so their streaks stayed grey in battle for five days.*

### 35. An overworld sheet with no rule of its own, cut the wrong way

***Symptom (2026-09-26, T-298):*** *Ty's new red-fox sheet was registered like every other graphics id and built clean. On a fresh map load he looked roughly right, but after his battle he vanished for the whole handover, standing or walking.* **Every overworld sheet needs its own line in `spritesheet_rules.mk`** (`-mwidth 2 -mheight 4` for a 16x32 figure). *Without one, `gbagfx` falls back to cutting the PNG in 8x8 tiles row by row across the whole 160px image, so "frame 0" is the top eight pixels of the first four figures (empty air), and only some frames happen to hold anything. Nothing warns.* ***And a fixed rule does not rebuild on its own***: *the `.4bpp` was already made, and once I deleted it the build still passed and kept the old bytes in the ROM, because nothing that included it was recompiled.* **After adding a rule, `make` the `.4bpp` by name and touch `src/event_object_movement.c`**; *then read the frames back out of the ROM (each 16x32 frame is 256 bytes from the sheet's symbol) before believing the screen. Two hours went into chasing sprite-copy queues first.* ***Also***: *a TRAINER's sheet has a tenth frame, ANIM_RAISE_HAND (the VS SEEKER's rematch pose); a sheet drawn from a non-trainer's nine reads past its own table there.*
...
- ***A warp lands you, not the story's walk.*** *A map whose scene runs on arrival (ONE ISLAND's first meeting with HOLT and CELIO) walks the player along a path that starts at the door, so a warp into the middle of the room ends that walk off the edge of the map.* **Warp to the door the story uses**, or warp twice once the scene has run.
- ***A scratch save made before 2026-09-25 has mGBA's clock record unset***, *so the day falls back to play time (trap 31) and the banner says SUNDAY — the game is right, the save is old.*
- ***To replay a leader's intro, clear the leader's trainer flag***: `TRAINER_FLAGS_START + TRAINER_…` *bit in `gSaveBlock1Ptr + 0xEE0`, and the next talk is the first. The song playing is `gMPlayInfo_BGM`'s first word — compare it with `gSongTable + 8 × MUS_…`.*
- ***To win a battle the test is not about, set `gBattleOutcome` to 1*** *(B_OUTCOME_WON) and take one turn: the engine finishes the battle as won and the script carries on — how the RECORD was played through (2026-09-25).*
- ***To put the player anywhere, use the theatre warp*** *(DEBUG build only, T-287)*: **`gDaemonsDebugWarp` is `{u8 armed, mapGroup, mapNum, unused; s16 x, y}`** — *poke the group (+1), number (+2), x (+4) and y (+6), then 1 into ARMED (+0), and the field warps there as a script's* `warp` *does once the player has control.* **A map's group and number are `MAP_…` in `include/constants/map_groups.h`, written `num | group << 8`.** *A battle's opponent can then be brought to 1 HP with* `poke16` *at `gBattleMons + 0x58 + 0x28`.*
