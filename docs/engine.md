...
**Set the theatre's routine with `poke16`, never by stepping to it.** *The address is `sTheatreMove` in `daemonsContent_debug.elf` (`arm-none-eabi-nm … | grep sTheatreMove`, and it moves with every build). The theatre ignores input while an animation plays, so a long animation swallows the D-pad steps after it: T-142's first sheet filmed the wrong routines from the fourth row down.* ***And reloading the script replays nothing***: *it now skips the batch already in the command file, after a reload re-ran a finished 620-command capture from the top.*
