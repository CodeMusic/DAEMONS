...
|---|---|---|
| [`vision.md`](vision.md) | **The living design bible.** Every decision, including reversed ones and the reasoning behind them. Updated as work proceeds. | v11.228, working |
| [`CONTEXT-CONTENT-design-bible-v11.228.pdf`](CONTEXT-CONTENT-design-bible-v11.228.pdf) | Typeset **snapshot** at v11.228 — the port, the bestiary, the harness, the fable cast, the streaks, every animation redrawn, the Index mark that is now the chart, which instrument draws whom, the twelve nobody had counted, the outline every picture of ours now carries, the canvas rule that comes with it, the two shoulder buttons, one of which is a perspective, and a true black outline on every picture of ours. Cut a new one with `./docs/build-pdf.sh 11.228` (the script adds the `v`). | frozen |
