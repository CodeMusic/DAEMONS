...
|---|---|---|
| [`vision.md`](vision.md) | **The living design bible.** Every decision, including reversed ones and the reasoning behind them. Updated as work proceeds. | v11.221, working |
| [`CONTEXT-CONTENT-design-bible-v11.221.pdf`](CONTEXT-CONTENT-design-bible-v11.221.pdf) | Typeset **snapshot** at v11.221 — the port, the bestiary, the harness, the fable cast, the streaks, every animation redrawn, the two tools that drew it as daemons, and the Index mark that is now the chart. Cut a new one with `./docs/build-pdf.sh 11.221` (the script adds the `v`). | frozen |
