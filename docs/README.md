...
|---|---|---|
| [`vision.md`](vision.md) | **The living design bible.** Every decision, including reversed ones and the reasoning behind them. Updated as work proceeds. | v11.219, working |
| [`CONTEXT-CONTENT-design-bible-v11.219.pdf`](CONTEXT-CONTENT-design-bible-v11.219.pdf) | Typeset **snapshot** at v11.219 — the port, the bestiary, the harness, the fable cast, the streaks, every animation redrawn, and the two tools that drew it as daemons. Cut a new one with `./docs/build-pdf.sh 11.219` (the script adds the `v`). | frozen |
