...
|---|---|---|
| [`vision.md`](vision.md) | **The living design bible.** Every decision, including reversed ones and the reasoning behind them. Updated as work proceeds. | v11.267, working |
| [`CONTEXT-CONTENT-design-bible-v11.267.pdf`](CONTEXT-CONTENT-design-bible-v11.267.pdf) | Typeset **snapshot** at v11.267 — the port, the bestiary, the harness, the fable cast, the streaks, every animation redrawn, the Index mark and the word that names it, the two shoulder buttons and the one rule they obey, the true black every picture of ours is outlined in, the driver that is installed rather than taught, the thing on the end of a shelf that writes in the margin, the pronoun that says the other one has an inside, the healthbox that names what persists and marks what does not, the rule that a margin can only answer an entry we wrote, the screen that was quietly teaching the wrong colour chart, the type named in its own colour on both halves of a dual type, the school that teaches a player how to read a file, the ninth driver that opens nothing, the first floor of the school, the notebook a man hands you because he believes in writing things down, a paper you can walk out of, a stairwell that doubles back, an exam season that still lets you leave, a room in the bought city that asks to see your diploma, two books that finally read like books, a school you can read from the street, a driver that is installed rather than taught, a tree that leads somewhere, the last three painted words that were not ours, an entry that can find its family, a town's music that stays in its town, a chart that is watched in both cartridges, a debug game that starts after the opening, a flag for every number, a follower measured before it is promised, the story told up to date, a document placed by one command, a PORT that withdraws again, an article that agrees with its name, a debug game that can jump to the diploma, one that comes back on a phone, one that starts in under two seconds, a line the bag had been cutting off, and a description the TOOLKIT was never showing. Cut a new one with `./docs/build-pdf.sh 11.267`. | frozen |
