...
|---|---|---|
| [`vision.md`](vision.md) | **The living design bible.** Every decision, including reversed ones and the reasoning behind them. Updated as work proceeds. | v11.223, working |
| [`CONTEXT-CONTENT-design-bible-v11.223.pdf`](CONTEXT-CONTENT-design-bible-v11.223.pdf) | Typeset **snapshot** at v11.223 — the port, the bestiary, the harness, the fable cast, the streaks, every animation redrawn, the Index mark that is now the chart, which instrument draws whom, and the twelve nobody had counted. Cut a new one with `./docs/build-pdf.sh 11.223` (the script adds the `v`). | frozen |
