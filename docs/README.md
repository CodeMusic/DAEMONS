...
|---|---|---|
| [`vision.md`](vision.md) | **The living design bible.** Every decision, including reversed ones and the reasoning behind them. Updated as work proceeds. | v11.233, working |
| [`CONTEXT-CONTENT-design-bible-v11.233.pdf`](CONTEXT-CONTENT-design-bible-v11.233.pdf) | Typeset **snapshot** at v11.233 — the port, the bestiary, the harness, the fable cast, the streaks, every animation redrawn, the Index mark and the word that names it, the two shoulder buttons and the one rule they obey, the true black every picture of ours is outlined in, the driver that is installed rather than taught, the thing on the end of a shelf that writes in the margin, the pronoun that says the other one has an inside, and the healthbox that names what persists and marks what does not. Cut a new one with `./docs/build-pdf.sh 11.233`. | frozen |
