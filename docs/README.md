...
|---|---|---|
| [`vision.md`](vision.md) | **The living design bible.** Every decision, including reversed ones and the reasoning behind them. Updated as work proceeds. | v11.232, working |
| [`CONTEXT-CONTENT-design-bible-v11.232.pdf`](CONTEXT-CONTENT-design-bible-v11.232.pdf) | Typeset **snapshot** at v11.232 — the port, the bestiary, the harness, the fable cast, the streaks, every animation redrawn, the Index mark and the word that names it, the canvas rule, the two shoulder buttons and the one rule they obey, the true black every picture of ours is outlined in, the Index refusing to show a daemon you have not met, the driver that is installed rather than taught, and the thing on the end of a shelf that writes in the margin. Cut a new one with `./docs/build-pdf.sh 11.232` (the script adds the `v`). | frozen |
