...
| [`type-chart.pdf`](type-chart.pdf) | **The chart on one sheet.** All 111 relations under our seventeen names, every type's one-clause test, why each colour — and the **colour roles, the HARDENED and OPAQUE hues and the move-menu marks, under review**. Generated by `python3 tools/gbachart.py --write`; cut with `./docs/build-pdf.sh type-chart.html`. | under review |
