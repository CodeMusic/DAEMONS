...
|---|---|---|
| [`vision.md`](vision.md) | **The living design bible.** Every decision, including reversed ones and the reasoning behind them. Updated as work proceeds. | v11.239, working |
| [`CONTEXT-CONTENT-design-bible-v11.239.pdf`](CONTEXT-CONTENT-design-bible-v11.239.pdf) | Typeset **snapshot** at v11.239 — the port, the bestiary, the harness, the fable cast, the streaks, every animation redrawn, the Index mark and the word that names it, the two shoulder buttons and the one rule they obey, the true black every picture of ours is outlined in, the driver that is installed rather than taught, the thing on the end of a shelf that writes in the margin, the pronoun that says the other one has an inside, the healthbox that names what persists and marks what does not, the rule that a margin can only answer an entry we wrote, the screen that was quietly teaching the wrong colour chart, the type named in its own colour on both halves of a dual type, the school that teaches a player how to read a file, and the ninth driver that opens nothing. Cut a new one with `./docs/build-pdf.sh 11.239`. | frozen |
