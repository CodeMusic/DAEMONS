...
- ***Ty's approach walks onto the player*** (T-286, **asked**): *after the battle he steps down into the only tile you can talk to him from — seen on screen, and his lines after it are said with his back to you.*
- ***A warp the theatre can fire*** (T-287, DEBUG build only): *what let the warehouse and FOLDS be played rather than read off the map;* **and `check_reach.py` now also checks every door and every flag the story waits on** *— against vanilla, with T-235's one known flag listed.* ***Ty's battle still calls him GIDEON*** *(T-288, asked).*
- ***The ending, played end to end in normal play*** *— warehouse, the Owl's PEER REVIEW, the flight, CRYSTAL on the island, home, the INDEX writing into the NOTEBOOK.* **It found the rating with no numbers** (T-289): *`port_dialogue.py` had dropped every value Gen 1 prints from a buffer, and a rewrite the sound vanilla ends a line with — seven leaders' music, VIRIDIAN's fanfare. Patched onto today's text; `check_reach.py` now guards both.* ***And the islands, played the same way***: *the first meeting with HOLT and CELIO, the SAPPHIRE into the machine, CELIO reading DOLDRUM off its log, and DOLDRUM CAVE open and read with no guard — the chain T-251 built, reached without JUMP for the first time.* ***And the RECORD***: *the REVIEW BOARD's champion, CRYSTAL's congratulations ("since you first left with ROVERCUB!"), the HALL, the credits, the title — all clean.* **The credits are still FireRed's, pictures and all, and four headings now credit real people with our work** (T-290, asked).
- ***PROTEUS stood on THE ANNEX as vanilla's DEOXYS*** (T-291): *the overworld palette tool never saw an object placed through a script variable, and its write mode had been broken since T-184 — now drawn from our art, and the 35 other overworld daemons take the true-black outline.*
- ***Every tool's write, run for real*** (T-292): *`check_generators.py --writes` runs them all in a sandbox; `tools/generator_drift.json` records the 24 whose write would change committed files, and why — most would undo later work. Three docs that were behind their generators are regenerated. Bringing the stale ones level is T-293.*
- ***The stale tools, level or fenced*** (T-293): **ten brought level** — *four wrote JASC palettes LF where git checks them out CRLF; `genmentors`, `genpairs` now draw T-177's outline through `gbaoutline.py`'s own pass (which keeps a 4-bit picture 4-bit); `genstates` and `genanims` leave a released animation alone instead of wrapping it as vanilla.* **Two were the tree behind the tool, and both were in the game**: *`streaks.h` had never been regenerated after the twelve daemons T-131 drew on 2026-09-20 (PILOT, TRITON, STARVED, CACHE and eight more), so their streaks stayed grey in battle — played, now coloured; and DARIO, the one PSYCHIC ♂ a player meets, still wore the jellyfish PSYCHIC ♀ had before T-178 redrew her.* **The other 14 carry hand work no tool knows, and now refuse a whole-tree `--write`** (`tools/driftguard.py`), *naming what they would change; `--over-later-work` writes anyway.*
- ***A new game in CONTEXT, played to the forest*** (T-294, T-295): *the controls, CRYSTAL's introduction, both names, the lab, the rival's battle, the CC-7 errand, the INDEX, the old man's demonstration and the first trainer in the forest, in the release ROM.* **Two bugs, fixed**: *four receipts were announced twice (the CC-7 three times in a row), carried from Gen 1, where the speaker says it; `check_reach.py` now catches that. And every trainer win read "Player outscored…", vanilla's literal word, above a line with the player's name.* **One question**: *about 20 of CRYSTAL's paragraphs and 40 of AL's are still OAK's and BLUE's with the words swapped — the whole first-battle tutorial among them (T-295).*
- ***CRYSTAL and AL in their own voices, drafted*** (T-295; *the user: "yes draft them all"*): **40 strings, 19 of CRYSTAL's and 21 of AL's, written and built as DRAFTS for the user's approval.** *CRYSTAL's whole first-battle tutorial ("Already. Of course it is." / "Two USERS. Two DAEMONS. Each one runs until the other halts."), the CC-7 greeting, the INDEX rating without "Wahaha!", and the ending: the champion's room, where she tells AL "You did everything correctly. I know. I have watched you do it.", and the HALL ("Names. Levels. That is what it keeps." / "Hold still. I am going to record you now."). AL at every rival battle after the lab, in the clipped register his Route 22 lines already had, and as champion: "It means I did it right."* **Played on new games in both editions**: *the greeting, the tutorial, the win lines, the CC-7 and AL's lab line.* **And one fix found doing it**: *the lab's gift read "put the USERBOXS in the BOXES POCKET" — vanilla pluralises by appending S to POKé BALL; a name ending in X now takes ES.*
...
- ***And the pushes after, fixes to the day's own work***: **the light changes while you stand in it** (T-276); **a clock nobody set is no clock** — *mGBA's zeroed record, a flash cart's 2000-01-01* (T-277); **a day banner never outlives its popup** (T-278); **the field effects take the light, and vanilla's inverted `LoadPal` tint is righted** (T-279); **L refuses an entry the INDEX cannot open yet** (T-280). **At night Routes 10 and 12 have water again** — *the night tables had grass only, so surfing and rods found nothing* (T-281); **the brain lights only on your own card** (T-282). **A cartridge clock in 12-hour mode is read, not refused** (T-265); **engine.md's ROM row counts the 16 MB file's 9.2 MB of contents**, as its paragraph already did. *Checked on CONTEXT too; a fresh clone builds both ROMs byte for byte.*
...
- ***Amended the same day, at the user's word***: **a chapter reads on one wide page, not across a spread** — *a spread held three words a line; the wide page holds about six, its paragraphs indented. CONTENTS keeps its two pages.*
...
- ***T-247: the HELP system's matchup pages were vanilla's***, **and never said CONTEXT and LATENT hit each other for double** — *vision.md 2's only change to the chart, and the chart is the argument.* **Both pages fixed, and `check_help_matchups()` now holds all 34 to `gTypeEffectiveness`.** *The STREAM lectures and every NPC matchup claim were checked the same way and are right.*
...
- ***T-244: SOURCE "powers up with MINUS"***, *and the AURORATICKET still sailed to BIRTH ISLAND.* **Ability and item descriptions sit in files the stale-name check read only for names.** *Both fixed, and the check now reads what an ability and an item say.*
...
- ***T-242: "I bound a UPTIME!"*** — **the article check called every U a consonant, and read one line at a time.** *Now "an UPTIME", and `check_articles` knows a U said "up" from one said "you" and reads across a break.*

---

## v11.269 — 2026-09-23

### The same fault, a word wider

- ***T-241: "by way of the THE BLACKOUT."*** — **a renamed place that begins with THE, after vanilla's own "the" on the line before.** *Fixed, gone from all four ROMs, and `check_lexicon` now refuses a word said twice across a break* — **four found, two deliberate and named, one this.**
