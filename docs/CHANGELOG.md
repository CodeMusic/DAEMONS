...
- ***The ending, played end to end in normal play*** *— warehouse, the Owl's PEER REVIEW, the flight, CRYSTAL on the island, home, the INDEX writing into the NOTEBOOK.* **It found the rating with no numbers** (T-289): *`port_dialogue.py` had dropped every value Gen 1 prints from a buffer, and a rewrite the sound vanilla ends a line with — seven leaders' music, VIRIDIAN's fanfare. Patched onto today's text; `check_reach.py` now guards both.*
