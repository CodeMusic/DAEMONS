...
| **T-39** | **`WIP — worktree-t39-relatives`.** **The cross-gen relatives of our Kanto lines are HALF renamed.** *Found by reading COLDREAD's line for T-36: SMOOCHUM is Jynx's pre-evolution, it is in the Index, it evolves into a daemon we named — and it is still called SMOOCHUM.* **It is not alone and it is not a clean scope boundary**: `CROBAT→MULTICAST`, `CLEFFA→NOISE`, `POLITOED→SEMAPHORE` and `SLOWKING→KERNEL` were all done, while ***PICHU, IGGLYBUFF, TYROGUE, SMOOCHUM, ELEKID, MAGBY, BELLOSSOM, ESPEON, UMBREON, STEELIX, SCIZOR, KINGDRA and BLISSEY were not.*** **Thirteen daemons hanging off lines that have names, without one.** *And the coherence sweep cannot see any of them* — **`gbacoherence.py` defines "ours" by DIFFING against upstream, so a daemon we forgot to rename is invisible to the check that would have caught it.** That is worth fixing in the tool whatever happens to the names | `engineGba`, species · `tools/gbacoherence.py` | T-36, 8.2 |
| **T-40** | **PULSAR still carries its VANILLA Index entry** — *a geometric body the locals suspect is alien*, on a daemon whose name is a timing signal and whose type is CONTEXT. **Nothing checks for this**: `port_index.py` wrote the entries we had, and the ones we never wrote kept Kanto's. *Same shape of gap as T-39 and probably the same size* — **count them before writing any** | `engineGba`, `pokedex_text_fr.h` | T-36 |
...
| **T-43** | **Two words in the lexicon carry a second reading nobody chose, and both look deliberate.** *Raised by the T-42 sweep so the call is recorded either way.* **`WEED`** is ODDISH — *a weed is plainly the dominant sense and the creature is one*, but the other reading exists. **`KILL`** is `GUILLOTINE` — **the genre has never used that word**, creatures *faint*; it is exactly right for a process world and it is still a deliberate tonal break. ***Neither is a leak of TAINT's kind*** — a leak is a meaning the design did not choose, and these look chosen. **This ticket exists to make that a decision instead of an oversight** | `engineGba`, species · moves | T-42 |
| **T-44** | **`check_agent_vocab` reports two stale names in the harness's LIVE prompt.** *`PRIORITY/Paralyze Heal` names a state we renamed to THROTTLED;* the second — *"backtracking confusion"* — **is ordinary English and a false positive**, which is worth fixing too: *a check that cries wolf on prose stops being read.* **The live prompt is agent runtime state, not a file we author**, so the question is whether it should be checked at all | `engineAi`, `tools/check_agent_vocab.py` | T-35, T-42 |
