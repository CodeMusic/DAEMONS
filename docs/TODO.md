...
| **T-38** | **`WIP — main`.** **Both story documents go stale silently.** *`story-readthrough.md` had one commit in eleven days and was a whole act behind; `story.md` existed only in a conversation until it was searched for in a transcript.* **Every other surface here has a tool or a check keeping it honest and these two have neither** — they are prose, so the check cannot be a diff. *A date and a "last reconciled against vX" line at the top would at least make the staleness visible* | `docs/`, `tools/` | this review |
