...
| | what | where | from |
|---|---|---|---|
| **T-46** | **The 52 island learnsets.** ***Naming them is what made them visible***: `gbacoherence` defines "ours" by whether a name was renamed, so T-45's fifty-two walked straight into its scope and **32 trip `SILENT TYPE` or `NO STAB`** — *vanilla Johto learnsets nobody has ever curated.* **Same job as T-36 and the same shape**: 2.7c's flags, read per daemon, with restoration where a line drops a second type's vocabulary. *Off-type overall is already 43% against 2.7a's 45% baseline, so this is coherence rather than coverage* | `level_up_learnsets.h` · `tools/gbacoherence.py` | T-45, 2.7c |
