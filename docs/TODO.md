...
| **T-48** | **The two regions are the last vanilla place names in the game.** *3.1 renamed eleven cities and 8.2a named twenty-one island places; `KANTO` and `SEVII` were never asked.* **Measured: `KANTO` is 3 `_()` strings and 12 dialogue lines; `SEVII` is `SEVII ISLANDS`, 14 `SEVII ISLE n` mapsecs and a handful of lines.** ***Blocked on the pick, not on the work*** — 0.6 recommends **`GAMUT`** and **`THE HOLDOUTS`** and says the pick is not its to make. **Generated headers half-apply a mapsec rename (engine trap 11): delete both and rebuild.** *Wants its own tool, beside `port_sevii.py`* | `src/strings.c`, `region_map_sections.json`, `data/maps/*/text.inc` | **0.6** |
