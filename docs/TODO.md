...
| **T-26** | **`WIP — main`. Sevii tier 3b — the tone rule, island by island.** ***FOUR ISLAND is done (`0bafc109f`) and it changed the ticket.*** **Five blocks of thirty-six**: the other thirty-one were *already* in island register, because vanilla's island NPCs were written loose and personal and the vocabulary pass carried them across intact. ***So 8.2a's rule is checkable on a single line — which is what this island was opened to find out — and 8.2a's SIZE is wrong.*** **Tier 3b is a READ of ~9,500 words and a rewrite of roughly a seventh of them.** *And the out-of-register lines are a **searchable population**, not a flat sweep:* **characters who carry mainland authority onto the islands** (the REVIEW BOARD's own leader was four of the five), **and signs and shopkeepers speaking for something larger than themselves.** *Remaining: ONE, TWO, THREE, FIVE, SIX, SEVEN* | `engineGba/data/maps/`, 6 islands left | 8.2a |
...
| **T-40** | **Vanilla Kanto Index entries under our names.** *Raised when PULSAR was found still describing a geometric body the locals suspect is alien.* **T-39 made it worse on purpose**: ***ten of the fourteen daemons just named carry vanilla text***, so the Index now says STATIC is small but can zap adults and CIRCULAR launches kicks while spinning. **Nothing checks for this** — `port_index.py` wrote the entries we had and the ones we never wrote kept Kanto's. *One thing measured and cleared while looking*: **the two editions are NOT drifting apart** — our FR and LG texts differ for 191 species where vanilla's differ for 252, *so the port reduced divergence rather than creating it*, and the edition question is closed. **What remains is authorship, not consistency** | `engineGba`, `pokedex_text_fr.h` · `_lg.h` | T-36, T-39 |
