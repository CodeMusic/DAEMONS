...
| **T-33** | **`WIP — main`.** **`MOON STONE` wants a name.** *Tied to a place rather than a paradigm, so 8.2 held it for the town pass* — **and the town pass is done.** The stone is already written into 4.30, so the name has to survive a room that describes it | `engineGba`, items | 8.2, 4.30 |
