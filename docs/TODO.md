...
| | what | where | from |
|---|---|---|---|
| **T-45** | **The 30 island daemons, in the PSYCHOLOGY register.** *8.2b's bound moved on 2026-09-11: **2.10 says name what the player can meet**, and these 30 have wild encounters while the other ~215 exist only in the data.* ***And all 30 are island-only — measured, zero on the mainland*** — **so they are Act 2's bestiary and take Act 2's register: Kanto's creatures are machinery, the islands' are mind.** *`HERACROSS`, `SKARMORY`, `LARVITAR`, `UNOWN`, `SNEASEL`, `MISDREAVUS`, `SMEARGLE`, `SHUCKLE` and twenty-two more.* **Each still needs its own line read for it** — 4.26's rule, and T-36's coherence flags apply once they are named | `engineGba`, species | 8.2b, 2.10 |
