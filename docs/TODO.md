...
| | what | closed by |
|---|---|---|
| **T-34** | ~~**The starter names are placeholders**~~ | this commit — ***seven of the nine were load-bearing and the ticket was stale***: 2.7a curated all three learnsets to the paradigms these names carry, `LABL` is 2.8's worked example for the punctuation rule, and there are 24 references outside the table. **But two of them really did look unfinished** — *checked against a dictionary, `LABL` and `CLUSTR` are the **only two non-words in 151 names***, and no engine ever forced it (`NAME_LENGTH` is 11 on the Game Boy and 10 on the GBA). ***Promoted rather than re-cut***: the two paradigms that learn **from a corpus** begin with a letter missing; the one that learns **by doing** is whole from the first day. *Which is why ROVERCUB is exempt without special pleading* |
