...
| | what | where | from |
|---|---|---|---|
| **T-27** | **The lab, post–Review Board: she is not there.** *A scientist hands over the Global Index; **the note she left is on the wall**.* **The machinery already exists and already fires at this exact moment** — `VAR_MAP_SCENE_PALLET_TOWN_PROFESSOR_OAKS_LAB` scene 7, an on-frame trigger, `removeobject` and `FLAG_HIDE_OAK_IN_HIS_LAB`. ***The note is the hard part and it is one string***: ordinary on its face, one clause that only parses if she expected not to be there, **and it must never change afterwards** (4.34's third ruling) | `data/maps/PalletTown_ProfessorOaksLab/` | 4.34 |
| **T-28** | **Brazen, the Owl — 4.24 staged.** You bring S.T.A.R.R. to Mr. Psychic's house and it argues her side out of her journals. ***Dialogue only — no document, no item, no flag the player can read.*** **The Owl concedes *something* and will not name it**, which is craft rule 1 said out loud by a character and **the one line in Act 2 that can wreck the game by being a sentence too generous** | `data/maps/SaffronCity_PokemonTrainerFanClub`-adjacent (Mr. Psychic's house) | 4.34, 4.23, 4.24 |
| **T-29** | **The flight, and the reading.** *The one thing only S.T.A.R.R. can do.* It takes you to her, you give her the package, **she reads it**, and you wake at home — *the same shape the Review Board transition already uses.* **Makes one dead map reachable** and so flips 2.10 for exactly that map, which then needs a name | `data/maps/BirthIsland`/`NavelRock`, `PalletTown_PlayersHouse_2F` | 4.34 |
| **T-30** | **The gold is not holding.** After the release, **more of `CLEAR LABORATORY` shows through the leaf on the Quicksilver sign.** *Nobody repainted it and nobody announces it* — 4.4 is explicit that the gold is a fact with a date, **so restoring it would be the game sneering at him**, which it never does. One `bg_event`, one extra string, gated on a flag | `data/maps/CinnabarIsland_PokemonLab`-adjacent (the Quicksilver lab sign) | 4.34, 4.4 |
