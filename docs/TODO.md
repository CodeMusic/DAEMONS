...
| **T-36** | **`WIP — worktree-t36-thirteen`** *(the thirteen SILENT TYPE; the tool and the first four are merged at `4d9cfdbc`)*. ***The tool is built, the four structural fixes are in*** (`tools/gbacoherence.py` 546c339 · learnsets 61d7d03e4) — **what remains is the thirteen SILENT TYPE and the forty-one THIN OPENING.** *Off-type overall came back at **44%** against 2.7a's 45% baseline, so the percentage was never the fault and the tool refuses to rank on it.* **NO STAB reports none.** *CHILLER and CRYOGEN were a whole line typed FLOW that knew no FLOW routine; CLAMPJAW was SWARM and never got the bonus once.* Five flags are excused in `EXEMPT`, keyed *(daemon, fault)* so a creature excused for one thing still reports the other two — **STUB knowing no routine of its own type IS the joke** (4.26). ***The remaining thirteen are second types that never arrived***: SCHEDULER, PIPELINE, ESCALATE, INJECTOR, PULSAR, COLDREAD, WISP, REVENANT, HAUNTPROC, TARPIT, STARR, ENSEMBLE, BADSEED. *`port_starters.py` and this commit's two lines are the pattern for the rest* | `tools/gbacoherence.py` · `level_up_learnsets.h` | 2.7, 2.7a, **2.7b** |
