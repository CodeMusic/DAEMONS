...
| **T-36** | ***The structural half is closed.*** **SILENT TYPE reports none and NO STAB reports none** — the tool (`tools/gbacoherence.py`), the first four (`61d7d03e4`) and the thirteen (`38d65ba08`) are all in, and *off-type overall sits at 42% against 2.7a's 45% baseline.* ***Eight of the thirteen were one fault***: a line carries both types and the routines for both, and **the last evolution drops the second type's whole vocabulary** — 4.26's PORYGON2 rule running backwards, so most of the pass was **restoration, not invention.** **What remains is the 41 THIN OPENING**, which is a *different and softer question*: everything before level 10 is off-type, and the first three things a daemon does are what teach a player what it is. *That one is judgement per daemon with no restoration shortcut available, so it wants its own sitting* | `tools/gbacoherence.py` · `level_up_learnsets.h` | 2.7, 2.7a, 2.7b, **2.7c** |
