...
| | what | where | from |
|---|---|---|---|
| **T-31** | **`PC` → `PORT`, decided in 1.7 and never swept.** *Four menu strings and ~25 dialogue lines.* ***The reason it has sat:*** **`\nPC` has no word boundary in front of it** — an escape is two characters and the first is a letter, so `\b` finds nothing and every `PC` at the start of a line is invisible to the pattern. **That is `engine.md`'s own trap and the tool already knows to flatten before matching** | `tools/port_vocab.py`, `src/`, `data/` | 1.7 |
| **T-32** | **Nineteen `type word + DAEMON` dialogue lines still name vanilla types**, nearly all in gyms. ***The hold is over*** — it was waiting on 5 to settle the Benchmark leaders and 5.3b built their mechanics. **Each one wants a human read rather than a substitution**: some are the type, some are the English word, and 1.2's whole point is that they are not the same job | `engineGba/data/maps/`, mostly gyms | 1.2, 5.3 |
