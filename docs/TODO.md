...
| | what | where | from |
|---|---|---|---|
| **T-41** | **`WIP — worktree-t41-openings`.** **The 41 thin openings.** *Everything a daemon knows before level 10 is off-type* — `BREAKER` at 89%, `FUSE` 88%, `SPAWN` and `UPSTREAM` 86%. ***T-36 closed the structural half and ruled this one out of it deliberately***: `SILENT TYPE` and `NO STAB` both report none, and the remaining question is softer. **The first three things a daemon does are what teach a player what it is**, and a line can be perfectly on-type by level 30 while opening as something else entirely. *There is no restoration shortcut here — T-36's thirteen were mostly a dropped second type being put back, and these are not.* **It is judgement per daemon, and `tools/gbacoherence.py <name>` reads one** | `level_up_learnsets.h` · `tools/gbacoherence.py` | 2.7c, T-36 |
