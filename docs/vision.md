...
**A total conversion — the living design bible, v11.277**
...
#### 2.7b The other sixty-nine, and the number that retired the question

**Built 2026-09-10** — `tools/gbacoherence.py`, T-36. *2.7a did three lines out of seventy-two by hand.* **This reads all 151 daemons that now carry one of our names**, and ***the first thing it said was that the premise had moved:*** **off-type overall is 44% against 2.7a's 45% baseline.** *The bestiary is sitting exactly on coverage.* **Ranking on the percentage would have produced a list of nothing**, so the tool reports it and refuses to rank on it.

***Which means the tool cannot score coherence, and does not try.*** **Whether ECHO invoking REFLECT reads right is a judgement, and nothing in a script can make it.** *So it finds the three faults that ARE computable — which are the three 2.7a found by hand — and then prints the learnset and gets out of the way.*

| | |
|---|---|
| **SILENT TYPE** | a daemon **has** a type and knows **not one routine of it**. *ROVERBYTE's exact fault: GROWTH/SIGNAL, with the SIGNAL half never appearing.* **A type the creature never uses is a label, not a design** |
| **NO STAB** | every damaging routine is off-type, so **the bonus never fires once** — *a daemon mechanically worse at everything it does than the chart says it is* |
| **THIN OPENING** | everything **before level 10** is off-type. ***The first three things a creature does are what teach a player what it is*** |

**19 · 4 · 41.** *And ROVERBYTE came back clean, because CARRIER is now in its list* — **the tool agreeing with a fix made by hand a day earlier is the only validation available and it is a good one.**

***A flag that is correct has to be able to stop reporting.*** **`EXEMPT` is keyed by *(daemon, fault)* rather than by daemon**, so *a creature excused for one thing still reports the other two* — and **each entry carries its ruling**, because a report that accumulates permanent known noise stops being read, and the day it stops being read is the day a real flag hides in it. **STUB is the case that proves it**: 4.26 calls it *"a placeholder that does nothing, until it does"*, and **a stub that knows no routine of its own type IS the joke.** *The tool flags it; a human says that.* **PENDING and BUFFER are cocoons with no damaging routine to make on-type, and CRAWLER is a larva with one attack that evolves at 7.**

#### The first three lines fixed — 2026-09-10

***CHILLER is typed FLOW and only FLOW, and its entire list was FROZEN and CONTENT.*** *Seel's vanilla set carried across untouched, Ice and Normal the whole way, on a daemon we retyped for water.* **The type defended, it was read in the Index, and it never once ACTED.**

**Two substitutions rather than one, because the two jobs sit at opposite ends of the curve.** `TRICKLE` takes level 1 from `COLLIDE` — *40 with the bonus is 60 against COLLIDE's 70, which is nothing*, and **the first routine is the one that teaches a player what the daemon is.** ***`DESCEND` could not sit there***: it would have been the only attack until 17, *and it spends a turn under water before it lands.* **So `DESCEND` takes 37 from `FORCE`**, where a charge turn is affordable — **the same 90 once the bonus applies, without the recoil** — and *it is FLOW's own clause as a verb, everything running downhill to the lowest point.* **A seal dives.** **CRYOGEN takes both**, at its own levels: *4.26's PORYGON2 rule, a line fixed at one stage is half a fix.*
...
#### Cleared 2026-09-10, and the pair was free

***`GROWTH` was a CONTENT move wearing the GRASS type's name, and `SWARM` was vanilla's Bug ability wearing the BUG type's*** — **the second found by `check_lexicon` on its first run** (8.7), which is the point of a check that reads every surface rather than the ones that existed when it was written.

**Both are the same operation seen from two sides, and the field already has the words:**

| | | |
|---|---|---|
| **`GROWTH`** move → **`SCALE UP`** | *raises one stat* | **scaling up is one instance given more** |
| **`SWARM`** ability → **`SCALE OUT`** | *powers up SWARM routines in a pinch* | **scaling out is more instances** |
...
***Why the type and not a silhouette.*** **The ENTRY prints species, category, height and weight and never once says the type** — *vanilla keeps it on the summary screen, and our Index carries it as chips on the LIST one screen earlier* — **so the mark is the only place the type reaches the page a player actually lingers on.** **And the chart is the argument** (8.4). *A silhouette was the alternative: at sixteen pixels in one bit it is mush, and it would say nothing the picture beside it does not already say.*
...
#### The first finding came from the empty file, not from the tool — 2026-09-12

***The channel was still empty at step 871, and the reason was a flaw in the fix.*** **Asking only on FIRST ARRIVAL is the best-quality moment and far too rare a one**: *the run's last new map was step 666*, **so 205 steps passed with nothing to trigger on** — *and it fails worst exactly where it matters, because* ***a stuck agent stops entering new maps and a stuck agent has the most to say.*** **Arrival now has a floor under it: sixty steps without an entry and it is asked about the last stretch anyway** (`engineAi` `ccaf769`). *Two counters, because one would re-ask every turn for as long as the agent declined, which is how a question becomes wallpaper.*

***But the run had already produced a finding, in `objectives.json`, where nobody was looking:***

> *"AERON alone still struggles vs **FLOW** gym. Need to level JINX, find a **Grass or Electric** type in tall grass…"*

**Two things in one sentence, and they point opposite ways.**

***It read FLOW off the screen and then reasoned in Grass and Electric.*** **The chart is legible enough to navigate — it knows what beats FLOW — and it is not learning that from us.** *It is remembering Pokémon.* **Which is 2.6's question arriving as evidence rather than as argument**: *can a player generalise, or only remember?* ***This reader remembered.***

***And then it ground Route 4 for 205 steps looking for something that is not there.*** **Measured off `wild_encounters.json`:**

| | |
|---|---|
| **Route 4 land** | PING · NIBBLE · WORM · PREEMPT · SECTOR — ***no GROWTH, no SIGNAL*** |
| **nearest GROWTH** | `WEED`/`SNARE` on **Route 5**, *which is past Doldrum* |

**That is vanilla's structure and not our bug** — *Route 4 has no Grass or Electric in FireRed either, and Misty expects the starter or the level.* ***The finding is the other half:*** **it wanted a GROWTH or SIGNAL daemon and could not tell from five names on Route 4 that none of them was one.** *It had to bind them and look.*

**Which is 2.6's closing paragraph, predicted and then observed:**
...
- **DOLDRUM CAVE: open the door, and let the player fail to see it** (*the user, 2026-09-23*). ***Today it is locked until the REVIEW BOARD is beaten and the islands' machine is finished*** — **"it goes to Doldrum Cave and waits for someone who can get through both gates"** (4.34). *The proposal keeps the second gate and makes it PERCEPTUAL: the player can walk in early, and without the understanding built on the islands the cave cannot be seen correctly — "it keeps changing and the environment itself doesn't make sense" — and an encounter is the unresolved LATENT thing, which speaks once and turns them away.* **The user's line for it: *HE WHO SEES ONE, SEES NONE. RETURN WHEN YOU THINK IN PERSPECTIVE.***
  - ***What it gets right***: **a lock says "not yet"; a cave you can enter and cannot read says what is missing without saying it** — *and the player arrives with the rock opera behind them and PERSPECTIVE already met, so the wall is made of the thing they have just been given.*
  - ***Three rulings it has to answer, all the bible's own.*** **(1) 8.6a rejected "the post-game item that colours the world" as *a Cognitive Clarifier for the player* — "an instrument that changes what they perceive, which is craft rule 1's worst violation."** *What may survive it: that ruling was against colour as a REWARD, handed over as an ITEM. An understanding that is earned by an act, and that changes one place, is closer to 8.6a's own PERSPECTIVE flash than to the thing it rejected.* **(2) 8.6a scopes grey to HALFTONE "by geography"** *— a grey cave would be the second grey place and may thin the first.* ***Claude's suggestion: make the cave wrong in its OWN way rather than Halftone's*** — **every tile and daemon present, and the ARRANGEMENT wrong** (*a layout that recomposes on each entry*): **content with no context**, *which is 4.24's sentence ("context does not transmit; only content does") drawn as a map.* **(3) Craft rule 1**: *"HE WHO SEES ONE, SEES NONE" is strong and says nothing aloud (it echoes Müller's "he who knows one knows none");* **"THINK IN PERSPECTIVE" names the thing, and PERSPECTIVE is already a routine's name.** *S.T.A.R.R.'s precedent is a place and a person, never the argument.*
  - ***Cost, cheapest first***: *a flag that swaps the cave's layout (vanilla already switches layouts by flag); one of N recomposed layouts picked on entry; looping warps. None needs new engine.*
- **UNDERSTANDINGS — the islands' DRIVERS, and a second thing the USER card holds** (*the user, 2026-09-23*). ***"Some of these items are not items but understandings — as you understand things you literally see them differently."*** **Items stay the computer-science side; understandings are the psychology side.** *Asked: are they collected like the MARKS, shown on the USER card, perhaps reserved for THE MARGINS — DRIVERS on the mainland, understandings on the islands — and is one of them what lets DOLDRUM CAVE be seen? Also asked: "I'd love to teach ABSTRACTION in the game somehow."*
  - ***It lands on three things already written.*** **4.3 and 4.24: "understanding does not survive a handoff; only method does" — "context does not transmit. Only content does."** *So the rule writes itself and is never said:* **a DRIVER is HANDED to you by a person; an understanding cannot be handed over, only arrived at** — *by an act, a witnessing, a battle.* **0.6 / 8.2b: GAMUT's subject is machinery and THE MARGINS' is mind**, *so DRIVERS on the mainland and understandings on the islands is the regions' own split.* ***And REVEAL is already one in all but name*** — **"the first driver that is a lens rather than a key", installed not used** (T-220).
  - ***The risk***: **a second row of collectibles on the USER card is a scoreboard, and 8.6a's reason for rejecting the colour item was "a reward is a metric — Scorn's kind of thing."** *Candidate answer: they are never counted and never listed as a tally; what a player has understood shows in how places LOOK, and the card shows only that something is there.*
  - ***Names***: **PERSPECTIVE and REVEAL are taken by routines, and `check_lexicon` forbids one word for two things** — *and ABSTRACTION named aloud is the thesis wearing a badge (craft rule 1).*
  - ***Scope (section 8)***: **a new mechanic across seven islands.** *Build DOLDRUM CAVE's one perceptual gate first, as the slice; decide the set after it has been played.*
- **What is HALFTONE Tower about, if not death?** (*the user, 2026-09-23*). ***"Since we are focusing on LATENT daemons it is not about ghosts any more … there should not be gravestones in there … and our sprites for these daemons are a bit too ghost-like"*** — **then, the same breath: *"it feels like a ghost because the content doesn't see the latent space, they only see its influence … maybe ghost fits better than I thought?"***
  - ***The bible already moved half of this.*** **4.5: Corpus there is "processing decommissioned daemons", and "Halftone is not a grief town. It is a town that does not know whether it is allowed to grieve."** *The dead are processes; the question is whether ending one is a death.*
  - ***Claude's reading***: **the user's second thought is the one to keep for the DAEMONS — LATENT is *running below the surface, unobserved* (2.6), and something known only by its influence is exactly what a ghost is to the people who report one.** *Keep the non-corporeal look; lose what is specifically FUNERARY.* ***And the stones may not have to go***: **a TOMBSTONE is a term of art — the marker a database keeps for a deleted record, so every copy learns it was deleted.** *A stone that is also a deletion record is 4.5's split in one object: half the town reads a grave, half a log entry, and both are at the wrong distance from it.* **If they stay, they change by what they SAY, not by what they are.**
  - ***The unresolved display*** (*vanilla's `$B8` ghost, seen without the RESOLVER*): **the most literal picture of latent space this project owns is its own sprite pipeline's** — *a diffusion model's latent at an early step: structured noise that only becomes a creature when decoded.* ***The RESOLVER is then the decoder.*** **Nothing says so.**
  - *And the Open item below — "does Halftone hold once the tower is written, or do Penumbra / Moiré serve better?" — is the same question from the town's side.*
