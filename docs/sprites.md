...
**Generated by `python3 tools/gbaspritecensus.py --write`; do not edit by hand.** *Measured against pret's `upstream/master` by pixel, at engine commit `d84615dcb`.* **Redrawn** means the drawing differs; **recoloured** is the same drawing with another palette in the PNG; **added** sprites vanilla never had and are kept out of the percentage. *The people line is the headline: every overworld figure and trainer picture a player sees, which is what 9.4's fable rule asks for.*
